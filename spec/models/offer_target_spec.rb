...
RSpec.describe OfferTarget, type: :model do
  describe 'validations' do
    ...
    it { should validate_inclusion_of(:gender).in_array(Player::GENDER_OPTIONS) }
    it { should validate_inclusion_of(:operating_system).in_array(Device::OPERATING_SYSTEMS) }
  ...
  describe 'offer_target_methods' do
    ...
    context '#to_s' do
      it 'should return attribute string for offer target' do
    ...
    context '#min_birtdate' do
      it 'should return min birtdate of offer target' do
        expect(offer_target.min_birtdate).to eq(offer_target.minimum_age.years.ago.to_date)
      end
    end

    context '#max_birtdate' do
      it 'should return max birtdate of offer target' do
